"""
OncoAI Fusion - Lung Cancer Classification Training
Multi-class classifier: Adenocarcinoma, Benign, Squamous Cell
"""
...
class Config:
    # Paths - Lung cancer data
    DATASET_ROOT = "/Users/lekhans/Desktop/Major-project/dataset_new"
    CHECKPOINT_DIR = Path(__file__).parent.parent / "checkpoints" / "lung_cancer"
...
# ============================================================================
# CUSTOM DATASET - Filter only lung cancer images
# ============================================================================

class LungCancerDataset(torch.utils.data.Dataset):
    """Dataset that filters only lung cancer images."""
    ...
    def __init__(self, root_dir, split='train', transform=None):
        ...
        print(f"  Loaded {len(self.images)} lung cancer images from {split}")
...
def train(args):
    print("=" * 60)
    print("🫁 OncoAI Fusion - Lung Cancer Classification")
    print("   Multi-class: Adenocarcinoma, Benign, Squamous Cell")
    print("=" * 60)
    print(f"\n📊 Configuration:")
    ...
    # Load lung cancer datasets
    print(f"\n📂 Loading lung cancer datasets...")
    train_dataset = LungCancerDataset(config.DATASET_ROOT, 'train', train_transform)
    val_dataset = LungCancerDataset(config.DATASET_ROOT, 'val', val_transform)
    ...
    model = ResNet50LungCancer(num_classes=config.NUM_CLASSES, pretrained=config.PRETRAINED)
    ...
    with open(config.LOG_DIR / f"lung_cancer_training_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json", 'w') as f:
        json.dump(history, f, indent=2)
    
    print("\n" + "=" * 60)
    print("✅ LUNG CANCER TRAINING COMPLETE!")
    print("=" * 60)
